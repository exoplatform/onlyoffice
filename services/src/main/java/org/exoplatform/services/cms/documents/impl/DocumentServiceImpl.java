...
public class DocumentServiceImpl implements DocumentService {
  ...
  private final List<NewDocumentTemplateProvider>   unmodifiebleTemplateProviders      =
                                                                                  Collections.unmodifiableList(templateProviders);

  private final List<DocumentEditorProvider>        unmodifiebleEditorProviders        =
                                                                                Collections.unmodifiableList(editorProviders);
  ...
  /**
   * Get link to open a document in the Documents application. This method will
   * try to guess what is the best drive to use based on the node path.
   * 
  ...
  /**
   * Get link to open a document in the Documents application with the given
   * drive
   * 
   * @param nodePath path of the nt:file node to open
  ...
  private String getFolderLinkInDocumentsApp(String nodePath, DriveData drive) throws Exception {
    ...
    if (drive.getName().startsWith(".spaces")) {
      ...
      if (groupId != null) {
        ...
        // the doc is in a space -> we use the documents application of the
        // space
        // we need to retrieve the root navigation URI of the space since it can
        // differ from
  ...
  /**
   * Retrieve the root navigation node URi of a space This method uses the
   * Portal Navigation API and not the Space API to avoid making ECMS depends on
   * Social
   * 
  ...
  protected String getSpaceRootNavigationNodeURI(String spaceGroupId) throws Exception {
    ...
    if (spaceNavigation != null) {
      ...
      if (navigationRootNode != null && navigationRootNode.iterator().hasNext()) {
        // we assume there is only one root navigation node, that's how spaces
        // work
  ...
  protected UserPortalConfig getDefaultUserPortalConfig() throws Exception {
    UserPortalConfigService userPortalConfigSer = ExoContainerContext.getService(UserPortalConfigService.class);
  ...
  private String getSharedLink(Node currentNode, Node rootNode) {
    ...
    try {
      ...
      nodes = linkManager.getNodeSymlinksUnderFolder(((ExtendedNode) currentNode).getIdentifier(),
                                                     shared.getPath(),
                                                     currentNodeWorkspaceName);
      if (!nodes.isEmpty()) {
        link = nodes.get(0);
      }
    ...
    if (template.getPath() != null && !template.getPath().trim().isEmpty()) {
      ...
      if (metadataPlugin != null && metadataPlugin.isExtensionSupported(template.getExtension())) {
        try {
          ...
          data = metadataPlugin.updateMetadata(template.getExtension(),
                                               data,
                                               new Date(),
                                               getCurrentUserDisplayName(userId),
                                               getCurrentUserLanguage(userId));
  ...
  public void saveCurrentDocumentProvider(String uuid, String workspace, String provider) throws RepositoryException {
    ...
    invokeUserSession(userId, (sessionProvider) -> {
  ...
  /**
   * Gets display name of current user. In case of any errors return current
   * userId.
   * 
        ...
        while (parentContentNode != null) {
          ...
          if (!parentPath.contains(driveHomePath)) {
            ...
            reversedFolderPathWithLinks.put(ManageDriveServiceImpl.PERSONAL_DRIVE_PUBLIC_FOLDER_NAME,
                                            getDocOpenUri(parentContentNode));
            break;
          }
  ...
  private String getDownloadUri(Node fileNode) throws Exception {
    ...
    downloadUrl.append('/')
               ...
               .append("/jcr/")
               .append(WCMCoreUtils.getRepository().getConfiguration().getName())
               .append('/')
               .append(workspaceName)
  ...
  private String getUserFullName(String userId) {
    ...
    // if the requested user is the connected user, get the fullname from the
    // ConversationState
    ...
    // if the requested user if not the connected user, fetch it from the
  ...
  /**
   * Invokes handler on node than can be locked. Retrieves lock token from
   * LockUtil and adds it to the session. Removes lock token from the session
  ...
  private void invokeUserSession(String userId, RepositoryConsumer<SessionProvider> handler) throws RepositoryException {
