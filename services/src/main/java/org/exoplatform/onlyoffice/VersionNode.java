...
public class VersionNode {
  ...
  public VersionNode(Node node, Session session) {
    ...
    try {
      ...
      uuid_ = node.getUUID();
      Property prop = getProperty(node, NodetypeConstant.EXO_LAST_MODIFIER);
  ...
  private Property getProperty(Node node, String propName) throws RepositoryException {
    ...
    } else if (node.hasNode(NodetypeConstant.JCR_FROZEN_NODE) && node.getNode(NodetypeConstant.JCR_FROZEN_NODE).hasProperty(propName)) {
      property = node.getNode(NodetypeConstant.JCR_FROZEN_NODE).getProperty(propName);
    }
    return property;
  }
  ...
  public Node getNode(String nodeName) throws RepositoryException {
    DMSConfiguration dmsConf = ExoContainerContext.getService(DMSConfiguration.class);
